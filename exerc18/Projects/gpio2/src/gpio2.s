...
;La chiffre de DÉBOUNCE en software
DELAI                   EQU     0X000A
...
main:   MOV R0, #(PORTN_BIT)
	...
	LDR R0, =GPIO_PORTN_BASE
        ...
        MOV R1, #00000011b ; Entrée en bit 0 et 1
...
Loop:	MOVT R6, #DELAI ; Donne la valeur du constant de debounce en software
        ...
        LDR R7, [R8, #0x00C]
        ;débounce par software: -------------------------------------------------------;
        ;tout d'abord on possède le constant de débounce, qui fait un loop décrementant;
        ;jusqu'à zéro, et dans ce loop on vérifie si le bouton est pressé ou nom. -----;
Debounce:        
        CBZ R6, OutDebounce
        SUB R6, #1
        B Debounce
